...
describe("EventCursor", () => {
  ...
  it("reset() clears the stored cursor", async () => {
    const server: any = {
    ...
    // After reset, the next scan should anchor again
    cursor.reset();
    await cursor.scan();
    expect(server.getLatestLedger).toHaveBeenCalledTimes(2);
  });

  it("handles empty event responses gracefully", async () => {
    const server: any = {
      getLatestLedger: jest.fn().mockResolvedValue({ sequence: 500 }),
      getEvents: jest.fn().mockResolvedValue({ events: [], latestLedger: 500 }),
    };

    const cursor = new EventCursor(server);
    const events = await cursor.scan({ fromLedger: 1 });
